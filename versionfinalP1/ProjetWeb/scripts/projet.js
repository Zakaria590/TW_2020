window.addEventListener('DOMContentLoaded', dessinerCarte);
...
function etatImage(etat){
  if (etat=="EN SERVICE")
    return '<img src="images/on.png" alt="image on">';
  else if (etat=="HORS SERVICE")
    return '<img src="images/off.png" alt="image off">';
  else
    return '<img src="images/maintenance.png" alt="image maintenance">';
}

function typeImage(type){
  if (type=="AVEC TPE")
    return '<img src="images/CB.jpeg" alt="avec CB">';
  else {
    return '<img src="images/noCB.png" alt="sans CB">';
  }
}

function informationBind(type,nom,adresse,commune,velos,places, etat){
    var informations = '<div class="informations">'
        +'<div class="images">'
        +etatImage(etat)
        +typeImage(type)
        +'</div>'
...
function placerMarqueurs(map){
  ...
  for (let item of document.querySelectorAll("#villes>li")){
    ...
    let adresse = item.dataset.adresse;
    let commune = item.dataset.commune;
    let etat = item.dataset.etat;
    let type = item.dataset.type;
    let velos =item.dataset.nbrvelos;
    let places = item.dataset.nbrplaces;

    let image =  VliveImage.getInstance(velos,places);

    let marker = L.marker(geoloc,{icon:image.getLeafletIcon()} )
    marker.addTo(map).bindPopup(informationBind(type,nom,adresse,commune,velos,places, etat));
    setupListeners(map,item, marker);
...
function setupListeners(maCarte,item, marker){
    item.addEventListener('click', ()=>{
      marker.openPopup();
      setCurrent(item);
      maCarte.setView(marker.getLatLng(),17);
    });
    marker.on("click", ()=>{
      setCurrent(item);
      maCarte.setView(marker.getLatLng(),17);
