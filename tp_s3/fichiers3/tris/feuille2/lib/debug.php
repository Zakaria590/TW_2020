...
*/
header("Content-Type: text/plain;charset=UTF-8");// type de fichier que le serveur va recevoir(moi)
...
function testReadBook($fileName){
    ...
    print_r($book);// on la trouve que dans le debugage, affiche ce quil y a dans la variable php (moi comme dump)
}
...
$TF = fopen('data/exempleLivre.txt','r');
echo bookToHTML(ReadBook($TF));
fclose($TF);
